...
class TkSheetTable(tk.Tk):
    def __init__(self,master,size, header,colWidth, bindings):
        self.master=master
        ...
        self.sheet = Sheet(self.master,
                            ...
                            height = height, #height and width arguments are optional
                            width = width #For full startup arguments see DOCUMENTATION.md
                            )
        #self.sheet.hide("row_index")
        #self.sheet.hide("header")
        #self.sheet.hide("top_left")
        bindings_basic=("single_select", #"single_select" or "toggle_select"
                                            ...
                                            "undo")
                                            #"edit_cell"))
        self.sheet.enable_bindings(bindings_basic+bindings)
        ...
        self.master.grid(row = 0, column = 0, sticky = "nswe")
        self.sheet.grid(row = 0, column = 0, sticky = "nswe")
        self.set_colums_width()
    ...
    def set_colums_width(self):
        for i in range(len(self.colWidth)):
            self.sheet.column_width(column = i, width = self.colWidth[i])
        #self.sheet.row_height(row = 0, height = 50)
    ...
    def add_row(self, idx, dt):
        if idx>self.rowCount:
            print('Error: row index insert out of range')
            return
        self.sheet.insert_row(values = (dt), idx = idx) # a filled row at the start
        self.rowCount+=1
...
if __name__=="__main__":
    ...
    group=tk.LabelFrame(root,text="Data table",bg='green')
    master=tk.Frame(group)
    header=["SerialNo1","SerialNo2","DataCheck","Result","Time"]
    header1=["Serial1","Serial2","Checking","Result1","Time1"]
    cWidth =[150,150,100,100,100]
    table = TkSheetTable(master,(700,300),header,cWidth,("edit-cell",""))
    table.add_row(table.rowCount,["1234566756","232345565","Data memory","OK","12:34:34"])
    table.add_row(table.rowCount,["3443445454","342232343","Data memory","NG","12:44:24"])
    table.sheet.headers(header1)
